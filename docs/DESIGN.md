...
| 理论 | 核心要点 | 对 Hive 的意义 | 学术验证 |
|------|---------|---------------|---------|
| **控制论** (Wiener) | 感知偏差 → 修正 → 收敛 | 测试不是验证代码对不对，是作为控制信号引导概率生成器收敛 | PID Control for LLMs (arXiv:2501.11979); 4/δ Bound (arXiv:2512.02080) 首证 LLM+验证器循环的收敛条件 |
| **有限理性** (Simon) | 别指望全局最优，"够好"就行 | 不逼 agent 一次写对，让它快速出一版，约束校正 | Satisficing Alignment (arXiv:2505.23729); Anthropic 2026 报告：仅 0-20% 任务可完全委托 |
| **痕迹协作** (Stigmergy) | 不直接通信，在共享环境留痕迹 | Agent 不互相对话（上下文炸）；读环境 → 做判断 → 写回环境 | Nature Comm Eng 2024: 数学证明 >3 agent 时间接协调优于直接通信 |

### 学术支撑

以下结论已被 2024-2026 学术研究实证（完整调研见 PKM）：

1. **评估器质量 > 生成器质量** — 错误信号的质量决定系统质量 (4/δ Bound, arXiv:2512.02080)
2. **类型约束在生成时缩小搜索空间** — TypeScript strict 让 LLM 编译错误减半 (ETH Zurich, arXiv:2504.09246)
3. **Property-Based Testing 捕获 30% unit test 漏过的 bug** — 是更好的控制信号 (FSE 2025)
4. **测试应作为 prompt 输入** — 不只是验证输出 (ASE 2024, TDD+LLM)
5. **SOP 约束 > 自由协作** — MetaGPT 85.9% Pass@1 (ICLR 2024)
6. **并行生成+选最优 > 串行重试** — AlphaCode 模式 (DeepMind)
7. **Spec-First 已成行业标准** — GitHub SpecKit + AWS Kiro + Thoughtworks 趋同
